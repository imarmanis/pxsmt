...
llvm::BasicBlock *LoopUnrollPass::makeDivergeBlock(llvm::Loop *l)
{
	...
	endLoopFun = parentFun->getParent()->getFunction("__VERIFIER_end_loop");
	//BUG_ON(!endLoopFun);
	assert(endLoopFun);
...
bool LoopUnrollPass::runOnLoop(llvm::Loop *l, llvm::LPPassManager &lpm)
{
    ...
    std::set<llvm::BasicBlock *> uniqueSuccBlocks(succBlocks.begin(), succBlocks.end());
...
llvm::LoopPass *createBLoopUnrollPass(int d)
{
	return new LoopUnrollPass(d);
}
